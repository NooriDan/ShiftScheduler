            ...
            <tbody>
                {
                    state.tas.map(ta => <tr key={ta.id} className="border">
                        <td className="p-2">{ta.name} (ID: {ta.id}) (requires: {ta.requiredShifts})</td>
                        ...
                        <td className="p-2">{state.shiftAssignments.filter(assignment => assignment.assignedTa && assignment.assignedTa.id === ta.id).map(assignment => assignment.shift.series).join(", ")}</td>
                    </tr>)
                }
            </tbody>
        </table>
